...
import not.savage.cereal.impl.config.Mode;
import not.savage.cereal.impl.config.sub.CacheConfig;
...
import not.savage.cereal.impl.platform.file.CerealFileDatabase;
import not.savage.cereal.impl.platform.mongo.CerealMongoDatabase;
import not.savage.cereal.impl.platform.sql.CerealSQLDatabase;
...
public class CerealAPI implements CerealLogger {
    ...
    public <T extends CerealCache<V>, V extends CerealDataBlob> T install(Class<T> cache, boolean distinct) {
            ...
            try {
                ...
                if (types.length > 0) {
                    if (types[0] instanceof Class<?>)
                        if (CerealDataBlob.class.isAssignableFrom((Class<?>) types[0]))
        ...
        try {
            this.sharedDatabaseInstance.loadOrCreateCache(cache, child, distinct);
        ...
        return (T) sharedDatabaseInstance.loadedCaches.get(cache);
    }
}
